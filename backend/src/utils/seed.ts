...
async function main() {
        ...
        data: [
            {
                ...
                priority: "High",
                citations: JSON.stringify([{ sourceId: (await prisma.source.findFirst({ where: { projectId: project.id, sourceType: 'manual' } }))?.id || '', chunkId: 'dummy', snippet: 'dummy snippet', confidence: 0.9 }]),
            },
            {
                projectId: project.id,
                ...
                priority: "Critical",
                citations: JSON.stringify([{ sourceId: (await prisma.source.findFirst({ where: { projectId: project.id, sourceType: 'document' } }))?.id || '', chunkId: 'dummy', snippet: 'dummy snippet', confidence: 0.9 }]),
            },
        ],
    });
    ...
    await prisma.conflict.create({
        data: {
            ...
            status: 'open',
            itemA: "Guest checkout required",
            itemB: "Users must be verified",
            sourceA: "manual_source_1",
            sourceB: "document_source_1",
        },
    });
